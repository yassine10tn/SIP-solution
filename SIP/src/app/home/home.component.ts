...
export class HomeComponent implements OnInit {
  ...
  selectedDossierFileName: string = '';

  onFileSelected(event: any) {
    const file = event.target.files[0];
  
    if (file) {
      this.selectedStatutsFileName = file.name;
      this.companyForm.get('statutsPdf')?.setValue(file);
  
      // Reset the input to hide "Choisir un fichier"
      event.target.value = '';
    }
  }
  selectedDossierFileNames: string[] = []; // Tableau pour stocker les noms des fichiers

  onDossierSelected(event: any) {
    const files = event.target.files;
    
    if (files && files.length > 0) {
      this.selectedDossierFileNames = Array.from(files).map((file: any) => file.name);
      this.companyForm.get('dossierJuridique')?.setValue(files);
  ...
  onSubmit() {
    ...
    if (this.companyForm.valid) {
      ...
      const convertedData = this.convertToIndices(formData); // Convertir les valeurs
  
      console.log('Données envoyées:', convertedData); // Log des données envoyées
  
      this.companyService.addCompany(convertedData).subscribe({
        next: (response) => {
          ...
          }, 2000); // Recharge la page après 1 seconde
        },
        error: (error) => {
          console.error('Une erreur est survenue !', error);
          this.snackBar.open('Erreur lors de l\'enregistrement des données. Veuillez réessayer.', 'Fermer', {
            duration: 3000,
            verticalPosition: 'bottom',
            horizontalPosition: 'right',
  ...
  private getCurrentStepControls(): string[] {
    switch (this.stepper.selectedIndex) {
      case 0:
        return ['exIdentifiant', 'raisonSociale', 'raisonSocialeAr', 'siegeSocial', 'siegeSocialAr'];
      case 1:
        return ['idPays', 'IdGouvernorat', 'MF'];
      case 2:
        return ['dateCreation', 'dateDossier', 'IdFormeJuridique', 'IdTypeProjet', 'status'];
      case 3:
        return ['objet', 'objetAr', 'IdSecteurEconomique'];
      case 4:
        return ['Telephone', 'Email'];
      case 5:
        return ['IdNatureProjet', 'idTypeManagement', 'identifiantSTB', 'IdTypeIdentifiant', 'idTypeEntreprise', 'idParametreRSM620', 'idParametreRNLPA870'];
      default:
        return [];
    }
  }
